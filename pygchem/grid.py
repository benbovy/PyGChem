...
class CTMGrid(object):
    ...
    @classmethod
    def _find_references(cls, model, references=None):
        """Iterate over model references and return a list of parent models."""
        references = references or []
        references.append(model)
        ref = cls._models[model].get('reference')
        if ref is not None:
            cls._find_references(ref, references)
    ...
    def get_model_info(cls, model_name):
        ...
        test_names = ("".join((n for n in itertools.chain(*zip(split_name, 
                                                               s + ('',)))))
                      for s in gen_seps)
        match_names = list(filter(lambda name: name in cls.models(),
                                  test_names))
        
        if len(match_names):
            model = match_names[0]   # take 1st model found (but more than 
                                     # one model should not occur)
            parent_models = cls._find_references(model)
            settings = dict()
