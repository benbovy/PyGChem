...
filter_results = ctm_f.filter(name="Ox", category="IJ-AVG-$")
tfield = filter_results[0].values

tfield = tfield[:,:,0:11].mean(axis=2).transpose()

tfield *= units_conv[mr_units]
...
# projection settings
# projected mesh
proj_latlon = tvtk.GeoProjection()
proj_rect = tvtk.GeoProjection(name='cea')
geo_trans = tvtk.GeoTransform(source_projection=proj_latlon,
                              destination_projection=proj_rect)


# vtk atmosphere thickness vs. earth radius: height scale calculation
atm_thickness = gchemgrid.c_km_geos5_r[-1] * 1e3
ratio_earth_atm = 2.0
...
continents_pts = continents_src.output.points.to_array()
continents_latlonr = xyz2latlonr(continents_pts)
continents_latlonr[:,2] = np.zeros((continents_pts.shape[0]))
spts2 = empty_like(continents_latlonr)
#geo_trans.transform_points(continents_latlonr, spts2)
#continents_src = tvtk.PolyData()
continents_src.output.points=continents_latlonr


writer = tvtk.XMLPolyDataWriter(input=continents_src.output,
                file_name=os.path.join(run_dir,"vtk","continents_latlon.vtp"))
writer.write()
...
#sgrid.cell_data.scalars = np.arange(vertices.shape[0]) #np.random.random(vertices.shape[0])
...
# retreive arrays
#sgrid.points.to_array()
#sgrid.cell_data.scalars.to_array()
#cells = sgrid.get_cells() --> return a CellArray object
#cells.to_array() --> return 1D numpy array
# (<nb_of_cell0_vertices>, <v0_1> ..., <nb_of_cell1_vertices>, <v1_1> ...)
