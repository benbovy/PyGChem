...
#
# BUILD GLOBCHEM.DB (SQLITE DATABASE)
#

# db scheme
db_path = os.path.join(ws, "globchem.db")
...
CREATE TABLE IF NOT EXISTS reac_kn (
reac_kn_id INTEGER PRIMARY KEY,
ordinal INTEGER,
line INTEGER,
status VARCHAR(1),
specific VARCHAR(1),
...
CREATE TABLE IF NOT EXISTS splinks (
...
for spec in species.keys():
    db_tuple = flatten_seq(species[spec][0:1]+[spec]+species[spec][1:])
    for string in [1,2,3,5]:
        db_tuple[string] = db_tuple[string].decode("utf-8")
    cursor.execute('INSERT INTO species VALUES \
                    (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', db_tuple)

rate_id = 0
for reac in reac_kn.keys():
    db_tuple = flatten_seq([reac]+reac_kn[reac][0:5]+reac_kn[reac][6:])
    for string in range(3,10)+range(11,42,2):
        db_tuple[string] = db_tuple[string].decode("utf-8")
    cursor.execute('INSERT INTO reac_kn VALUES \
                    (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,\
                     ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,\
                     ?,?)', db_tuple)
    rate_line = 0
    for rate in reac_kn[reac][5]:
        db_tuple = [rate_id,reac,rate_line]+rate
...
for link in splinks:
    cursor.execute('INSERT INTO splinks VALUES \
                        (?,?,?,?)', link)
